...
interface ModelSpec {
    ...
    /** Published list price per 1M input tokens. `null` when IronBridge does not know it. */
    inputCost: number | null;
    /** Published list price per 1M output tokens. `null` when IronBridge does not know it. */
    outputCost: number | null;
    /** Published context window in tokens. `null` when IronBridge does not know it. */
 ...
 * reference sources".
 *
 * They came from nowhere. IronBridge runs no benchmark and records no timing;
...
export default function Comparison() {
    ...
    // providers. A discovered model gets nulls, not defaults: IronBridge does not
            ...
            for (const provider of apiProviders) {
                ...
                for (const modelName of provider.models) {
                    models.push({
                        ...
                        // fact. A cloud model IronBridge has no entry for is unknown.
        ...
        <div className="space-y-6">
            ...
            <div className="flex items-start gap-2 text-sm text-[hsl(var(--muted-foreground))] bg-[hsl(var(--muted))]/50 rounded-lg p-4">
                ...
                <p>
                    ...
                    Published list prices and context windows, compiled into the app. IronBridge does not
                    ...
                    <span className="font-mono">{UNKNOWN}</span> where IronBridge has no published figure
                    for them.
                </p>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                ...
                <div className="bg-[hsl(var(--card))] rounded-xl p-4 border">
                    ...
                    <p className="text-sm text-[hsl(var(--muted-foreground))] mt-1">
                        {unknownCount > 0
                            ? `${unknownCount} with figures IronBridge does not have`
            ...
            <div className="bg-[hsl(var(--card))] rounded-xl p-6 border">
                ...
                {unknownCount > 0 && (
                    <p className="text-xs text-[hsl(var(--muted-foreground))] mt-2 flex items-center gap-1">
                        ...
                        plotted: IronBridge has no published figure for {unknownCount === 1 ? 'it' : 'them'}.
                    </p>
                )}
            </div>
