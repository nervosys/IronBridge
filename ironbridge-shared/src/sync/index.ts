...
// =============================================================================
// IRONBRIDGE Sync Service
// =============================================================================
// Real-time data synchronization between ironbridge-rust, ironbridge-web, and ironbridge-app
...
export class SyncService {
    ...
    private loadCachedState(): void {
        try {
            const cached = localStorage?.getItem(`ironbridge-sync-state-${this.config.clientId}`);
            if (cached) {
    ...
    private saveCachedState(): void {
        try {
            localStorage?.setItem(
                `ironbridge-sync-state-${this.config.clientId}`,
                JSON.stringify({
    ...
    clearCache(): void {
        try {
            localStorage?.removeItem(`ironbridge-sync-state-${this.config.clientId}`);
        } catch (err) {
