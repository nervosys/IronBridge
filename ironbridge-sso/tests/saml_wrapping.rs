...
use base64::Engine;
use ironbridge_sso::saml::{self, SamlConfig};
...
fn sign_assertion(idp: &Idp, assertion: &str, id: &str) -> String {
    ...
    let canonical = ironbridge_sso::saml::c14n::canonicalize(doc.root_element(), &[]);
    let digest =
    ...
    let si_canonical = ironbridge_sso::saml::c14n::canonicalize(si_doc.root_element(), &[]);
    let signature_value =
