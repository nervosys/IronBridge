...
//! # Where files land
//!
//! `CHASM_DOWNLOAD_DIR` if set, otherwise a `downloads` directory beside the
...
//! spare, or if it exceeds `CHASM_MAX_DOWNLOAD_BYTES`. A download that fills
...
/// Ceiling on a single file, overridable with `CHASM_MAX_DOWNLOAD_BYTES`.
...
fn download_root(db_path: &Path) -> PathBuf {
    if let Ok(dir) = std::env::var("CHASM_DOWNLOAD_DIR") {
...
fn max_download_bytes() -> u64 {
    std::env::var("CHASM_MAX_DOWNLOAD_BYTES")
        .ok()
...
pub async fn start_download(state: State, body: web::Json<StartDownloadRequest>) -> HttpResponse {
    ...
    if size > cap {
        return bad_request(format!(
            "That file is {size} bytes, over the {cap}-byte limit. Raise \
             CHASM_MAX_DOWNLOAD_BYTES on the server to allow it."
        ));
    }
    ...
    match free_space_for(&dest) {
        Some(free) if free < size.saturating_add(FREE_SPACE_RESERVE) => {
            return bad_request(format!(
                "That file is {size} bytes and only {free} are free. Chasm keeps \
...
mod tests {
    ...
    fn the_download_root_defaults_beside_the_database() {
        std::env::remove_var("CHASM_DOWNLOAD_DIR");
        let root = download_root(Path::new("/var/lib/chasm/chasm.db"));
        assert!(root.ends_with("downloads"), "{}", root.display());
        assert!(root.starts_with("/var/lib/chasm"), "{}", root.display());
    }

    #[test]
    fn the_size_cap_falls_back_to_the_default() {
        std::env::remove_var("CHASM_MAX_DOWNLOAD_BYTES");
        assert_eq!(max_download_bytes(), DEFAULT_MAX_BYTES);

        std::env::set_var("CHASM_MAX_DOWNLOAD_BYTES", "not a number");
        assert_eq!(max_download_bytes(), DEFAULT_MAX_BYTES);

        std::env::set_var("CHASM_MAX_DOWNLOAD_BYTES", "0");
        assert_eq!(max_download_bytes(), DEFAULT_MAX_BYTES, "zero disables");

        std::env::set_var("CHASM_MAX_DOWNLOAD_BYTES", "1024");
