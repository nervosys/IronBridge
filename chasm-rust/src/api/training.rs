...
fn no_provider() -> HttpResponse {
    fail(
        ...
        "No training provider configured. Set OPENAI_API_KEY (and \
         OPENAI_BASE_URL for a compatible endpoint) on the server. Chasm does \
...
mod tests {
    ...
    /// Submitting without a provider is a 503 that says Chasm does not train.
    #[tokio::test]
