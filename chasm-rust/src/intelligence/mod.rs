...
impl InsightsGenerator {
    ...
    pub fn generate(&self, session: &ChatSession) -> ConversationInsights {
        ...
        ConversationInsights {
            ...
            questions: user_msgs
                .iter()
                .flat_map(|m| m.split('.').filter(|s| s.contains('?')))
                .map(String::from)
                .collect(),
...
impl SentimentAnalyzer {
    pub fn new() -> Self {
        Self {
            positive: vec!["good", "great", "thanks", "helpful", "works"]
                .into_iter()
                .map(String::from)
                .collect(),
            negative: vec!["bad", "error", "fail", "wrong", "bug"]
                .into_iter()
                .map(String::from)
                .collect(),
        }
    ...
    pub fn analyze(&self, session: &ChatSession) -> Sentiment {
        ...
        let score = if total > 0 {
            (pos as f32 - neg as f32) / total as f32
        } else {
            0.0
        };
        ...
        Sentiment {
            ...
            confidence: if total > 0 { 0.8 } else { 0.5 },
        }
    }
}
...
impl QualityScorer {
    ...
    pub fn score(&self, session: &ChatSession) -> QualityScore {
        ...
        let completeness = if session.assistant_responses().is_empty() {
            0
        } else {
            80
        };
        QualityScore {
            overall: (clarity + completeness) / 2,
            clarity,
            completeness,
        }
    }
