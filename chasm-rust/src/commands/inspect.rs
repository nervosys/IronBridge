...
//! `chasm inspect` — Low-level inspection of VS Code state databases, session
...
fn validate_session_file(
    ...
    if size < 500 {
        ...
        if backup.exists() {
            ...
            issues.push(format!(
                "Backup exists ({} bytes) — recoverable with 'chasm register repair'",
                backup_size
            ));
        }
...
/// `chasm inspect rebuild` — Rebuild session index and model cache from files
