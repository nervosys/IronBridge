...
Start the REST API server to integrate IronBridge with web apps, mobile apps, or custom tooling.
...
are in the spec, each marked `x-ironbridge-feature: enterprise`. The spec tests
...
**Since SAML moved to `ironbridge-sso`, that first command works everywhere.** The
...
- `GET /oidc/callback` consumes a single-use login state, so it can never
  answer 200 to a cold probe. It carries `x-ironbridge-probe: needs-prior-state`,
...
> **This section is now history.** `samael` was replaced by `ironbridge-sso`, which
...
- Harvest and export are CLI-only (`ironbridge harvest`, `ironbridge export`).
