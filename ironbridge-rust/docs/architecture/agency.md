...
```rust
use ironbridge::agency::{Tool, ToolSchema};
...
```bash
ironbridge agency run --agent coder "Fix the authentication bug"
```
...
```bash
ironbridge agency run --orchestration sequential "Build and test a REST API"
```
...
```rust
use ironbridge::agency::{ProactiveMonitor, PermissionLevel};
