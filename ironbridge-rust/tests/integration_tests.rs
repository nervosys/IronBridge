...
//! Integration tests for Chat System Manager (ironbridge)
//!
...
mod cli_tests {
    ...
    fn test_cli_run_tui_command() {
        let cli = Cli::try_parse_from(["ironbridge", "run", "tui"]).unwrap();
        assert!(matches!(cli.command, Commands::Run { .. }));
    }

    #[test]
    fn test_cli_list_workspaces_command() {
        let cli = Cli::try_parse_from(["ironbridge", "list", "workspaces"]).unwrap();
    ...
    fn test_cli_find_workspace_command() {
        let cli = Cli::try_parse_from(["ironbridge", "find", "workspace", "my_project"]).unwrap();
        assert!(matches!(cli.command, Commands::Find { .. }));
    }

    #[test]
    fn test_cli_find_session_command() {
        let cli = Cli::try_parse_from(["ironbridge", "find", "session", "rust"]).unwrap();
        assert!(matches!(cli.command, Commands::Find { .. }));
    }

    #[test]
    fn test_cli_show_path_command() {
        let cli = Cli::try_parse_from(["ironbridge", "show", "path", "/path/to/project"]).unwrap();
        assert!(matches!(cli.command, Commands::Show { .. }));
    }

    #[test]
    fn test_cli_fetch_path_command() {
        let cli = Cli::try_parse_from(["ironbridge", "fetch", "path", "/path/to/project"]).unwrap();
        assert!(matches!(cli.command, Commands::Fetch { .. }));
    }

    #[test]
    fn test_cli_merge_path_command() {
        let cli = Cli::try_parse_from(["ironbridge", "merge", "path", "/path/to/project"]).unwrap();
        assert!(matches!(cli.command, Commands::Merge { .. }));
    }

    #[test]
    fn test_cli_show_workspace_command() {
        let cli = Cli::try_parse_from(["ironbridge", "show", "workspace", "test-project"]).unwrap();
        assert!(matches!(cli.command, Commands::Show { .. }));
    }

    #[test]
    fn test_cli_show_session_command() {
        let cli = Cli::try_parse_from(["ironbridge", "show", "session", "session-id-123"]).unwrap();
        assert!(matches!(cli.command, Commands::Show { .. }));
    }
    ...
    fn test_cli_git_init_command() {
        let cli = Cli::try_parse_from(["ironbridge", "git", "init", "/path/to/project"]).unwrap();
        assert!(matches!(cli.command, Commands::Git { .. }));
    }

    #[test]
    fn test_cli_git_add_command() {
        let cli = Cli::try_parse_from(["ironbridge", "git", "add", "/path/to/project"]).unwrap();
        assert!(matches!(cli.command, Commands::Git { .. }));
    }

    #[test]
    fn test_cli_git_status_command() {
        let cli = Cli::try_parse_from(["ironbridge", "git", "status", "/path/to/project"]).unwrap();
        assert!(matches!(cli.command, Commands::Git { .. }));
    }
    ...
    fn test_cli_provider_list_command() {
        let cli = Cli::try_parse_from(["ironbridge", "provider", "list"]).unwrap();
        assert!(matches!(cli.command, Commands::Provider { .. }));
    }

    #[test]
    fn test_cli_provider_info_command() {
        let cli = Cli::try_parse_from(["ironbridge", "provider", "info", "ollama"]).unwrap();
        assert!(matches!(cli.command, Commands::Provider { .. }));
    }
    ...
    fn test_cli_provider_test_command() {
        let cli = Cli::try_parse_from(["ironbridge", "provider", "test", "ollama"]).unwrap();
        assert!(matches!(cli.command, Commands::Provider { .. }));
    }
    ...
    fn test_cli_help_flag() {
        ...
        let result = Cli::try_parse_from(["ironbridge", "--help"]);
        assert!(result.is_err()); // Help flag causes early exit
    }

    #[test]
    fn test_cli_version_flag() {
        let result = Cli::try_parse_from(["ironbridge", "--version"]);
...
mod workspace_tests {
    use super::*;
    use ironbridge::workspace::normalize_path;
...
mod error_tests {
    ...
    fn test_error_display_workspace_not_found() {
        let error = IronBridgeError::WorkspaceNotFound("/test/path".to_string());
    ...
    fn test_error_display_session_not_found() {
        let error = IronBridgeError::SessionNotFound("session-123".to_string());
        let msg = format!("{}", error);
    ...
    fn test_error_display_vscode_running() {
        let error = IronBridgeError::VSCodeRunning;
        let msg = format!("{}", error);
