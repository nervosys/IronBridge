...
use ironbridge::providers::config::{IronBridgeConfig, ProviderConfig, ProviderType};
use ironbridge::providers::session_format::{GenericMessage, GenericSession};
use ironbridge::providers::ProviderRegistry;
...
// ============================================================================
// IRONBRIDGE Config Tests
// ============================================================================

mod ironbridge_config_tests {
    use super::*;

    #[test]
    fn test_ironbridge_config_default() {
        let config = IronBridgeConfig::default();
    ...
    #[test]
    fn test_ironbridge_config_serialization() {
        let mut config = IronBridgeConfig {
    ...
    #[test]
    fn test_ironbridge_config_deserialization() {
        let json = r#"{
            "providers": [
        ...
        let config: IronBridgeConfig = serde_json::from_str(json).unwrap();
    ...
    #[test]
    fn test_ironbridge_config_find_provider() {
        let mut config = IronBridgeConfig::default();
        config
            .providers
            .push(ProviderConfig::new(ProviderType::Ollama));
        config
...
mod openai_compat_tests {
    use super::*;
    use ironbridge::providers::openai_compat::{OpenAIChatMessage, OpenAICompatProvider};
    use ironbridge::providers::ChatProvider;
...
mod cursor_provider_tests {
    use super::*;
    use ironbridge::providers::cursor::CursorProvider;
    use ironbridge::providers::ChatProvider;
...
mod integration_tests {
    ...
    fn test_provider_config_integration() {
        ...
        let json = serde_json::to_string_pretty(&config).unwrap();
        let restored: IronBridgeConfig = serde_json::from_str(&json).unwrap();
        ...
        let handles: Vec<_> = (0..4)
            .map(|i| {
                thread::spawn(move || {
                    ...
                    for _ in 0..100 {
                        let json = serde_json::to_string(&config).unwrap();
                        let _: IronBridgeConfig = serde_json::from_str(&json).unwrap();
                    }
                    i
                })
            })
