//! Basic usage examples for IRONBRIDGE library
//!
//! Run with: cargo run --example basic_usage

use ironbridge::models::ChatSession;
use ironbridge::workspace::{discover_workspaces, find_workspace_by_path};

fn main() -> anyhow::Result<()> {
    println!("=== IRONBRIDGE Basic Usage Examples ===\n");
    ...
    for ws in workspaces.iter().take(5) {
        println!(
            "   - {} | {} sessions | {}",
            ironbridge::text::head(&ws.hash, 12),
        ...
        "requests": [
            {
                ...
                "message": { "text": "How do I use IRONBRIDGE?" },
                "response": { "value": [{"value": "IRONBRIDGE helps manage chat sessions!"}] }
            }
        ]
    }"#;
