...
- **`tests/module_reachability.rs`** — fails if any `.rs` file in any of the
  three crates (`ironbridge-rust`, `ironbridge-sso`, `ironbridge-desktop`) is not reachable
...
- **Every local LLM provider reported itself running** — `is_available` was
  ...
  default, so `ironbridge provider list` claimed Ollama, vLLM, LM Studio, LocalAI,
...
- **`ironbridge shard` command** — Split oversized sessions into linked shard files
  - `ironbridge shard session <file>` — Shard a single session file
  - `ironbridge shard workspace` — Shard all oversized sessions in a workspace
  - `ironbridge shard info <file>` — Show linked-list shard metadata
...
- **CLI Name Standardization** - Binary renamed from `ironbridge` to `ironbridge`
- **CLI Polish** - Shell completions, `ironbridge doctor`, provider guides
- **Sync Command** - Bidirectional session backup and restore (`ironbridge sync`)
- **Watch Command** - File-system monitoring for session changes (`ironbridge watch`)
- **Agent Launcher** - `ironbridge run agent` with auto-save capability
...
- Moved `ironbridge run list-agents` → `ironbridge list agents`
...
- **Release Readiness** - Added `[lib]` section to Cargo.toml (lib target: `ironbridge`)
- **Feature Declaration** - Added `[features]` section with `enterprise` feature
- **Integration Tests** - Fixed `use ironbridge::...` imports (previously broken by missing lib target)
- **MCP Binary Import** - Updated `ironbridge-mcp` to use `ironbridge` lib target
- **Version Test** - Updated `test_version_flag` to expect `ironbridge` instead of `ironbridge`
...
- **MCP Server - IRONBRIDGE Database Integration** - Access ironbridge-web database sessions
  - 5 new `ironbridge_db_*` tools for ironbridge-web database access:
    - `ironbridge_db_list_workspaces` - List workspaces from IRONBRIDGE database
    - `ironbridge_db_list_sessions` - List sessions with provider/workspace filters
    - `ironbridge_db_get_session` - Get session with all messages
    - `ironbridge_db_search` - Search sessions by title
    - `ironbridge_db_stats` - Database statistics by provider
  - 3 new `ironbridge://db/*` resources:
    - `ironbridge://db/workspaces` - Workspaces resource
    - `ironbridge://db/sessions` - Sessions resource  
    - `ironbridge://db/stats` - Statistics resource
    - `ironbridge://db/session/{id}` - Individual session resource

### Changed

- MCP server now supports both VS Code workspace storage AND ironbridge-web database
...
- **VS Code ALL SESSIONS Support** - Access workspace-independent sessions
  - `ironbridge list workspaces` now shows "Empty window sessions (ALL SESSIONS)" count
  - `ironbridge list sessions` includes ALL SESSIONS with "(ALL SESSIONS)" as project path
...
- **Harvest System** - Unified database for collecting sessions from all providers
  - `ironbridge harvest init` - Initialize the harvest database
  ...
  - `ironbridge harvest status` - Show database statistics
  - `ironbridge harvest list` - List harvested sessions
  - `ironbridge harvest export` - Export sessions from the database

- **Share Link Import** - Import shared chat sessions from web URLs
  - `ironbridge harvest share <url>` - Register a share link for import
  - `ironbridge harvest shares` - List pending or imported share links
  - Supports ChatGPT, Claude, Gemini, Perplexity, and Poe share URLs

- **Session Checkpoints** - Version snapshots for session tracking
  - `ironbridge harvest checkpoint <session>` - Create a named checkpoint
  - `ironbridge harvest checkpoints <session>` - List session checkpoints
  - `ironbridge harvest restore <session> <checkpoint>` - Restore to checkpoint

- **Full-Text Search** - Search across all harvested messages
  - `ironbridge harvest search <query>` - Search with FTS5 or LIKE fallback
...
- **Auto-Detection Improvements**
  - `ironbridge detect` - Enhanced workspace and provider detection
  - `ironbridge detect all` - Comprehensive detection report
  - `ironbridge detect providers --with-sessions` - Filter by active providers
