...
pub fn telemetry_opt_in() -> Result<()> {
    ...
    if config.enabled {
        ...
        config.opt_in()?;
        // Deliberately not "thank you for helping improve IronBridge". Nothing is
        // sent to Nervosys -- this flag only permits `ironbridge telemetry record`
    ...
    if endpoint.is_none() && headers.is_none() {
        ...
        if path.exists() {
            ...
            println!(
                ...
                "ironbridge telemetry setup".cyan()
            );
        }
        return Ok(());
    }
