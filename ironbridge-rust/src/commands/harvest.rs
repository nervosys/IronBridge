...
pub fn harvest_init(path: Option<&str>, git_init: bool) -> Result<()> {
    ...
    if db_path.exists() {
        ...
        println!("   Use 'ironbridge harvest run' to update it");
        return Ok(());
    }
    ...
    println!("\nNext steps:");
    println!("  1. Run 'ironbridge harvest scan' to see available providers");
    println!("  2. Run 'ironbridge harvest run' to collect sessions");
    if !git_init {
        println!("  3. Run 'ironbridge harvest git init' to enable version tracking");
    }
...
pub fn harvest_status(path: Option<&str>) -> Result<()> {
    ...
    if !db_path.exists() {
        ...
        println!("   Run 'ironbridge harvest init' to create one");
        return Ok(());
    }
            ...
            .unwrap_or(false)
    {
        ...
        match output {
            Ok(out) => {
                ...
                if status.is_empty() {
                    ...
                    println!("   Run 'ironbridge harvest git commit' to save changes");
                }
            }
            Err(_) => {
        ...
        println!("   Run 'ironbridge harvest git init' to enable version tracking");
    }
...
fn get_db_path(path: Option<&str>) -> Result<PathBuf> {
    ...
    if let Ok(p) = std::env::var("IRONBRIDGE_HARVEST_DB") {
        return Ok(PathBuf::from(p));
    }
...
/// On a machine that has never run `ironbridge harvest`, `ChatDatabase::open` would
...
fn init_git_tracking(db_path: &Path) -> Result<()> {
    ...
    println!("   Run 'ironbridge harvest git commit -m \"message\"' to save changes");
    println!("   Run 'ironbridge harvest git log' to view history");
...
pub fn harvest_share(
    ...
    println!("   3. Import with: ironbridge import <file>");
    println!();
    println!("   Or use 'ironbridge harvest shares' to view pending links.");
...
pub fn harvest_shares(
    ...
    if !db_path.exists() {
        anyhow::bail!("Harvest database not found. Run 'ironbridge harvest init' first.");
    }
    ...
    if rows.is_empty() {
        ...
        println!("   Use 'ironbridge harvest share <url>' to add a share link");
        return Ok(());
    }
...
pub fn harvest_checkpoint(
    ...
    let (internal_id, actual_session_id, msg_count) = match session {
        ...
        None => {
            ...
            println!("   Use 'ironbridge harvest list' to see available sessions");
    ...
    println!();
    println!(
        "   Use 'ironbridge harvest checkpoints {}' to view history",
        session_id
    );
...
pub fn harvest_checkpoints(db_path: Option<&str>, session_id: &str) -> Result<()> {
    ...
    if checkpoints.is_empty() {
        ...
        println!(
            "   Use 'ironbridge harvest checkpoint {} -m \"message\"' to create one",
            session_id
        );
        return Ok(());
    }
    ...
    println!();
    println!(
        "   Use 'ironbridge harvest restore {} <checkpoint>' to restore",
        session_id
    );
...
pub fn harvest_restore_checkpoint(
    ...
    let (snapshot_json, original_msg_count) = match checkpoint {
        ...
        None => {
            ...
            println!(
                "   Use 'ironbridge harvest checkpoints {}' to see available checkpoints",
                session_id
            );
