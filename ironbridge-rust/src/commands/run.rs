...
//! and automatically harvests the resulting session into the ironbridge database.
    ...
    let config = resolve_agent(alias).ok_or_else(|| {
        anyhow::anyhow!(
            "Unknown agent '{}'. Supported agents:\n\
             ...
             \n\nRun 'ironbridge list agents' to see all agents.",
            alias,
            "*".cyan(),
            "*".cyan(),
            "*".cyan(),
            ...
            if new_sessions.is_empty() {
                ...
                match auto_harvest_sessions(&new_sessions) {
                    ...
                    Err(e) => {
                        ...
                        println!(
                            "{} Run 'ironbridge harvest run' manually to save sessions",
                            "[i]".blue()
                        );
                    }
                }
...
pub(crate) fn auto_harvest_sessions(session_files: &[std::path::PathBuf]) -> Result<usize> {
    ...
    let db_path = if let Ok(p) = std::env::var("IRONBRIDGE_HARVEST_DB") {
    ...
    harvest_run(
        ...
        true,
        false,
        Some("Auto-save from ironbridge run"),
        false,
    )?;
