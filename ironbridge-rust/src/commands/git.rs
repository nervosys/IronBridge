...
pub fn git_init(project_path: &str) -> Result<()> {
    ...
    println!(
        "  2. Or commit them: ironbridge add {} --commit -m 'Add chat sessions'",
        project_path
    );
...
pub fn git_add(project_path: &str, commit: bool, message: Option<&str>) -> Result<()> {
    ...
    if !chat_sessions_path.exists() {
        anyhow::bail!(
            "Chat versioning not initialized. Run 'ironbridge init {}' first",
            project_path
        );
    }
...
pub fn git_track(
    ...
    if !chat_sessions_path.exists() {
        anyhow::bail!(
            "Chat versioning not initialized. Run 'ironbridge git init {}' first",
            project_path
        );
    }
...
pub fn git_log(project_path: &str, count: usize, sessions_only: bool) -> Result<()> {
    ...
    println!();
    println!(
        "{} Use 'ironbridge git diff --from <hash>' to see changes",
        "[i]".cyan()
    );
