...
pub fn recover_from_recording(
    ...
    if let Some(sid) = session_id {
        ...
        if let Some(arr) = sessions.get("active_sessions").and_then(|v| v.as_array()) {
            ...
            println!();
            println!(
                "[i] Use 'ironbridge recover recording --session <ID>' to recover a specific session"
            );
        } else {
...
pub fn recover_from_database(
    ...
    if let Some(sid) = session_id {
        ...
        println!();
        println!(
            "[i] Use 'ironbridge recover database {} --session <ID>' to export a session",
            backup_path
        );
    }
                    ...
                    if file_path.extension().is_some_and(|e| e == "jsonl") {
                        if let Some((req_count, total_chars)) = is_gutted_session(file_path) {
                            ...
                            println!(
                                ...
                                "ironbridge recover backups".cyan()
                            );
                        }
                    }
                }
    ...
    if output_json {
        ...
        match format_info.format {
            VsCodeSessionFormat::LegacyJson => {
                ...
                println!(
                    "    - Convert to JSONL: ironbridge recover convert \"{}\" --format jsonl",
                    file
                );
            }
            VsCodeSessionFormat::JsonLines => {
                ...
                println!(
                    "    - Convert to JSON: ironbridge recover convert \"{}\" --format json",
                    file
                );
            }
        }
        println!(
            "    - Export to Markdown: ironbridge recover convert \"{}\" --format md",
            file
        );
    }
...
pub fn recover_copilot_info(session_dir: Option<&str>, output_json: bool) -> Result<()> {
    ...
    if output_json {
        ...
        if has_errors || has_warnings {
            ...
            if has_warnings {
                println!(
                    "    [?] Run 'ironbridge recover upgrade' to update legacy sessions to JSONL format"
                );
                println!("    [?] Run 'ironbridge doctor --fix' to repair any session issues");
            }
        } else {
