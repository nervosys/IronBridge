...
pub fn history_fetch(project_path: Option<&str>, force: bool, no_register: bool) -> Result<()> {
    ...
    println!(
        "{} {} sessions",
        "Fetched:".truecolor(171, 157, 242),
        fetched_count.to_string().truecolor(252, 152, 103)
    );
    if skipped_count > 0 {
        println!(
            "{} {} (use --force to overwrite)",
            "Skipped:".truecolor(171, 157, 242),
            skipped_count.to_string().truecolor(252, 152, 103)
        );
    }
    ...
    if fetched_count > 0 && !no_register {
        ...
        if is_vscode_running() && !force {
            ...
            println!("   Run 'ironbridge fetch path --force' after closing VS Code to register.");
        } else {
            let registered =
...
pub fn merge_sessions_by_list(
    ...
    if found_sessions.is_empty() {
        ...
        println!(
            "\n{} Tip: Use 'ironbridge list sessions' or 'ironbridge find session <pattern>' to find session IDs",
            "[i]".cyan()
        );
        return Ok(());
    }
    ...
    println!(
        "\n   {}: {} sessions found",
        "Total".truecolor(171, 157, 242),
        found_sessions.len().to_string().truecolor(252, 152, 103)
    );
