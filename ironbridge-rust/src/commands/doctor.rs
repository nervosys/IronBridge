...
//! `ironbridge doctor` — Environment diagnostics and health checks
...
/// — `ironbridge doctor` sat silent for over ten minutes and was indistinguishable
...
pub fn doctor(full: bool, format: &str, fix: bool, quick: bool) -> Result<()> {
    ...
    if streaming {
        println!();
        println!("  {}", "IronBridge Doctor".bold().cyan());
...
fn check_version() -> CheckResult {
    ...
    CheckResult::pass("system", "IronBridge version").with_detail(&format!("v{version}"))
}
...
fn check_harvest_db() -> CheckResult {
    ...
    match db_path {
        ...
        Some(p) => CheckResult::warn(
            ...
            "Harvest database",
            &format!(
                "Not found at {}. Run `ironbridge harvest run` to create it.",
                p.display()
            ),
        ),
...
fn check_git() -> CheckResult {
    match std::process::Command::new("git").arg("--version").output() {
        ...
        _ => CheckResult::warn(
            "tools",
            "Git",
            "Not found in PATH (optional, needed for `ironbridge git`)",
        ),
    }
}
...
fn get_harvest_db_path() -> Option<PathBuf> {
    dirs::data_dir().map(|p| p.join("ironbridge").join("harvest.db"))
}
...
mod printer_tests {
    ...
    fn a_result_is_written_as_soon_as_it_is_emitted() {
        ...
        printer.emit(&CheckResult::pass("system", "IronBridge version"));

        let written = String::from_utf8(printer.out.clone()).unwrap();
        assert!(
            written.contains("IronBridge version"),
