...
//! Command implementations for `ironbridge sdk` subcommands.
...
/// `ironbridge sdk list` — Show the languages a client can be generated for
pub fn sdk_list(json: bool) -> Result<()> {
    ...
    println!(
        ...
        "IRONBRIDGE_BASE_URL".yellow(),
        "IRONBRIDGE_API_KEY".yellow()
    );

    Ok(())
}

/// `ironbridge sdk generate` — Write a client for one language, or for all of them
pub fn sdk_generate(
...
mod tests {
    ...
    fn generate_refuses_to_overwrite_without_force() {
        let dir = tempdir().unwrap();
        let out = dir.path().to_str().unwrap();
        let path = dir.path().join("ironbridge.go");
    ...
    fn a_batch_collision_writes_nothing() {
        let dir = tempdir().unwrap();
        let out = dir.path().to_str().unwrap();
        std::fs::write(dir.path().join("IronBridge.java"), "// mine").unwrap();
        ...
        assert!(
            !dir.path().join("ironbridge.py").exists(),
            "the batch wrote a file despite aborting"
        );
        assert_eq!(
            std::fs::read_to_string(dir.path().join("IronBridge.java")).unwrap(),
            "// mine"
        );
    }
    ...
    fn base_url_is_baked_in_without_a_trailing_slash() {
        ...
        sdk_generate(
            ...
            Some(out),
            false,
            "https://ironbridge.example.com/",
            false,
        )
        .unwrap();

        let body = std::fs::read_to_string(dir.path().join("ironbridge.rb")).unwrap();
        assert!(body.contains("https://ironbridge.example.com'"));
        assert!(!body.contains("ironbridge.example.com/'"));
    }
