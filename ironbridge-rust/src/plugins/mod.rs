...
//!
//! Provides an extensible plugin architecture for IRONBRIDGE:
...
pub struct PluginManifest {
    ...
    /// Minimum IRONBRIDGE version required
    pub ironbridge_version: String,
    /// Plugin entry point
    pub main: String,
...
impl PluginManager {
    ...
    fn validate_manifest(&self, manifest: &PluginManifest) -> Result<()> {
        ...
        if !req.matches(&current) {
            return Err(anyhow!(
                "Plugin requires IRONBRIDGE {}, but current version is {}",
                manifest.ironbridge_version,
                current_version
            ));
        }
