...
//! MCP Resources - Expose ironbridge data as MCP resources
...
pub fn list_resources() -> Vec<Resource> {
    vec![
        ...
        Resource {
            uri: "ironbridge://workspaces".to_string(),
            name: "VS Code Workspaces".to_string(),
            description: Some("All VS Code workspaces with chat sessions".to_string()),
            mime_type: Some("application/json".to_string()),
        },
        Resource {
            uri: "ironbridge://sessions".to_string(),
            name: "VS Code Sessions".to_string(),
            description: Some("All chat sessions from VS Code workspaces".to_string()),
            mime_type: Some("application/json".to_string()),
        },
        Resource {
            uri: "ironbridge://orphaned".to_string(),
            name: "Orphaned Sessions".to_string(),
            description: Some("Sessions on disk but not in VS Code's index".to_string()),
            mime_type: Some("application/json".to_string()),
        },
        Resource {
            uri: "ironbridge://providers".to_string(),
            name: "Providers".to_string(),
            // "Available" read as "installed here", which this is not: it is
            // a static catalogue of the provider kinds IronBridge can harvest.
            // `ironbridge_detect` is the tool that answers what is actually present.
            description: Some(
                "Provider kinds IronBridge can harvest from (a static catalogue, not a scan of this machine)"
                    .to_string(),
            ),
            mime_type: Some("application/json".to_string()),
        },
        // IRONBRIDGE Database resources (ironbridge-web)
        Resource {
            uri: "ironbridge://db/workspaces".to_string(),
            name: "IRONBRIDGE-Web Workspaces".to_string(),
            description: Some("Workspaces from the ironbridge-web database".to_string()),
            mime_type: Some("application/json".to_string()),
        },
        Resource {
            uri: "ironbridge://db/sessions".to_string(),
            name: "IRONBRIDGE-Web Sessions".to_string(),
            description: Some("Chat sessions from the ironbridge-web database".to_string()),
            mime_type: Some("application/json".to_string()),
        },
        Resource {
            uri: "ironbridge://db/stats".to_string(),
            name: "IRONBRIDGE-Web Statistics".to_string(),
...
pub fn read_resource(uri: &str) -> ReadResourceResult {
    match uri {
        // VS Code workspace resources
        "ironbridge://workspaces" => read_workspaces_resource(),
        "ironbridge://sessions" => read_sessions_resource(),
        "ironbridge://orphaned" => read_orphaned_resource(),
        "ironbridge://providers" => read_providers_resource(),
        // IRONBRIDGE Database resources (ironbridge-web)
        "ironbridge://db/workspaces" => read_db_workspaces_resource(),
        "ironbridge://db/sessions" => read_db_sessions_resource(),
        "ironbridge://db/stats" => read_db_stats_resource(),
        _ => {
            // Try to parse dynamic URIs
            if let Some(hash) = uri.strip_prefix("ironbridge://workspace/") {
                read_workspace_resource(hash)
            } else if let Some(id) = uri.strip_prefix("ironbridge://session/") {
                read_session_resource(id)
            } else if let Some(id) = uri.strip_prefix("ironbridge://db/session/") {
...
fn read_orphaned_resource() -> ReadResourceResult {
    ...
    ReadResourceResult {
        contents: vec![ResourceContent {
            ...
            mime_type: Some("application/json".to_string()),
            text: Some(json!({
                "message": "Use ironbridge_list_orphaned tool with a specific path to find orphaned sessions"
            }).to_string()),
            blob: None,
        }],
    }
...
fn read_workspace_resource(hash: &str) -> ReadResourceResult {
    ...
    match discover_workspaces() {
        Ok(workspaces) => {
            for ws in &workspaces {
                if ws.hash.starts_with(hash) || ws.hash == hash {
                    ...
                    return ReadResourceResult {
                        contents: vec![ResourceContent {
                            uri: format!("ironbridge://workspace/{}", ws.hash),
                            mime_type: Some("application/json".to_string()),
                            text: Some(
                                serde_json::to_string_pretty(&json!({
                                    "hash": ws.hash,
            ...
            ReadResourceResult {
                contents: vec![ResourceContent {
                    uri: format!("ironbridge://workspace/{}", hash),
                    mime_type: Some("text/plain".to_string()),
...
fn read_session_resource(session_id: &str) -> ReadResourceResult {
    ...
    match discover_workspaces() {
        Ok(workspaces) => {
                ...
                if let Ok(sessions) = get_chat_sessions_from_workspace(&ws.workspace_path) {
                    for s in sessions {
                        ...
                        if sid.starts_with(session_id) || sid == session_id {
                            ...
                            return ReadResourceResult {
                                contents: vec![ResourceContent {
                                    uri: format!("ironbridge://session/{}", sid),
                                    mime_type: Some("application/json".to_string()),
                                    text: Some(
                                        serde_json::to_string_pretty(&json!({
                                            "id": sid,
            ...
            ReadResourceResult {
                contents: vec![ResourceContent {
                    uri: format!("ironbridge://session/{}", session_id),
                    mime_type: Some("text/plain".to_string()),
...
// ============================================================================
// IRONBRIDGE Database Resource Implementations (ironbridge-web integration)
// ============================================================================

fn read_db_workspaces_resource() -> ReadResourceResult {
    use super::db;

    if !db::ironbridge_db_exists() {
        return ReadResourceResult {
            contents: vec![ResourceContent {
                uri: "ironbridge://db/workspaces".to_string(),
                mime_type: Some("application/json".to_string()),
                text: Some(
                    json!({
                        "error": "IRONBRIDGE database not found",
                        "message": "Initialize ironbridge-web database first",
                        "db_path": db::get_ironbridge_db_path().display().to_string()
                    })
                    .to_string(),
                ),
                blob: None,
                ...
                contents: vec![ResourceContent {
                    ...
                    text: Some(
                        serde_json::to_string_pretty(&json!({
                            "workspaces": infos,
                            "total": infos.len(),
                            "source": "ironbridge-web database"
                        }))
                        .unwrap_or_default(),
                    ),
                    blob: None,
...
fn read_db_session_resource(session_id: &str) -> ReadResourceResult {
    ...
    if !db::ironbridge_db_exists() {
        return ReadResourceResult {
            contents: vec![ResourceContent {
                uri: format!("ironbridge://db/session/{}", session_id),
                mime_type: Some("application/json".to_string()),
                text: Some(
                    json!({
                        "error": "IRONBRIDGE database not found"
