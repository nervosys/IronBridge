...
//! MCP Database Access - Connect to the IRONBRIDGE database for ironbridge-web integration
//!
//! This module provides read-only access to the IRONBRIDGE database, enabling the MCP server
//! to expose ironbridge-web's chat sessions without modifying VS Code's workspace storage.
...
/// Get the default IRONBRIDGE database path
pub fn get_ironbridge_db_path() -> PathBuf {
    dirs::data_local_dir()
        .map(|p| p.join("ironbridge").join("ironbridge.db"))
        .unwrap_or_else(|| PathBuf::from("ironbridge.db"))
}

/// Check if the IRONBRIDGE database exists
pub fn ironbridge_db_exists() -> bool {
    get_ironbridge_db_path().exists()
}

/// Open the IRONBRIDGE database (read-only mode)
pub fn open_ironbridge_db() -> Result<ChatDatabase> {
    let path = get_ironbridge_db_path();
    ChatDatabase::open(&path)
}

/// List all workspaces from the IRONBRIDGE database
pub fn list_db_workspaces() -> Result<Vec<Workspace>> {
    let db = open_ironbridge_db()?;
    db.list_workspaces()
}
...
/// List sessions from the IRONBRIDGE database
