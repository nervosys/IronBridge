...
//! in [`ironbridge_sso::oidc`]. This module is the part that has to exist in the
//! server: provider configuration, the pending-login store, and turning a
//! verified [`ironbridge_sso::Identity`] into a IronBridge user and session.
//!
...
impl OidcProviderConfig {
    ...
    fn to_sso_config(&self) -> ironbridge_sso::oidc::OidcConfig {
        ironbridge_sso::oidc::OidcConfig {
            issuer: self.issuer.clone(),
            client_id: self.client_id.clone(),
...
impl OidcService {
    ...
    pub async fn complete_login(
        ...
        let auth_request = ironbridge_sso::oidc::AuthRequest {
