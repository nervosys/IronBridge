...
/// Get system prompt with IRONBRIDGE tools context
pub async fn get_ironbridge_system_prompt() -> impl Responder {
    use crate::mcp::tools::list_tools;

    let tools = list_tools();
    let tool_descriptions: Vec<String> = tools
        .iter()
        .filter(|t| t.name.starts_with("ironbridge_db_")) // Only database tools for chat
        .map(|t| {
            format!(
                "- {}: {}",
                t.name,
    ...
    let system_prompt = format!(
        r#"You are an AI assistant integrated with the Chat Session Manager (IRONBRIDGE) system. You have access to tools that let you introspect and query the user's chat history database.

Available IRONBRIDGE tools:
{}
...
Guidelines:
- Use ironbridge_db_list_workspaces to see all projects/workspaces with chat sessions
- Use ironbridge_db_list_sessions to see chat sessions, optionally filtered by workspace or provider
- Use ironbridge_db_get_session to retrieve full conversation history from a specific session
- Use ironbridge_db_search to search across session titles
- Use ironbridge_db_stats to get overview statistics
...
pub const MASTER_KEY_ENV: &str = "IRONBRIDGE_MASTER_KEY";
...
/// key derived from `IRONBRIDGE_MASTER_KEY` and this install's salt.
///
...
mod credential_encryption_tests {
    ...
    /// `IRONBRIDGE_MASTER_KEY` is process-wide, so these tests must not run at the
