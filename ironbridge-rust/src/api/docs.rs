...
mod tests {
    ...
    /// this test, that page documented `ironbridge_list_workspaces`,
    /// `ironbridge_get_session`, `ironbridge_search_sessions` and `ironbridge_get_stats`, and
    /// told the reader to run `ironbridge mcp serve`. Not one of those names has
    /// ever existed: the prefix is `ironbridge_`, two of the four have no counterpart
    /// under any prefix, and `ironbridge` has no `mcp` subcommand. An agent
    ...
    fn the_mcp_reference_documents_exactly_the_tools_that_exist() {
        ...
        // Headings of the form: #### `ironbridge_something`
    ...
    ///
    /// `ironbridge mcp serve` was the documented way to start the server for as
    /// long as the page existed. The binary is `ironbridge-mcp`.
    #[test]
    fn the_mcp_reference_names_the_binary_that_exists() {
        assert!(
            MCP_DOC.contains("ironbridge-mcp"),
            "docs/api/mcp.md should name the ironbridge-mcp binary"
        );
        for line in MCP_DOC.lines() {
            let trimmed = line.trim();
            // Allow the paragraph that explains the old, wrong invocation.
            if trimmed.contains("ironbridge mcp") && !trimmed.contains("There is no") {
                assert!(
                    trimmed.contains("no `ironbridge mcp`") || trimmed.contains("`ironbridge mcp serve`"),
                    "docs/api/mcp.md still instructs `ironbridge mcp`, which is not a subcommand: {trimmed}"
                );
            }
        }
    }
    ...
    fn gated_out(item: &serde_json::Value) -> bool {
        item.get("x-ironbridge-feature").and_then(|f| f.as_str()) == Some("enterprise")
            && !cfg!(feature = "enterprise")
    }
    ...
    async fn documented_response_bodies_match_what_the_server_sends() {
        ...
        for (path, item) in paths {
            ...
            // offline cases.
            if op.get("x-ironbridge-probe").and_then(|v| v.as_str()) == Some("needs-network") {
                continue;
            }
            ...
            if op.get("x-ironbridge-probe").and_then(|v| v.as_str()) == Some("needs-prior-state") {
