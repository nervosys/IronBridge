...
def _default_base_url() -> str:
    ...
    return os.environ.get("IRONBRIDGE_BASE_URL") or "{{base_url}}"


def _default_api_key() -> Optional[str]:
    """The API key from the environment, or None when unauthenticated."""
    return os.environ.get("IRONBRIDGE_API_KEY") or None
...
pub const RUST_SDK_TEMPLATE: &str = r##"// IronBridge Rust SDK
// Auto-generated - Do not edit directly
// Version: {{version}}
//
// Add to Cargo.toml:
...
impl Default for Config {
    fn default() -> Self {
        Self {
            base_url: std::env::var("IRONBRIDGE_BASE_URL")
                .unwrap_or_else(|_| "{{base_url}}".to_string()),
            api_key: std::env::var("IRONBRIDGE_API_KEY").ok().filter(|k| !k.is_empty()),
...
impl std::fmt::Display for Error {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            Error::Http(e) => write!(f, "ironbridge: transport error: {e}"),
            Error::Api { status, message } => write!(f, "ironbridge: API error {status}: {message}"),
        }
    }
}
...
impl Client {
    pub fn new(config: Config) -> Result<Self, Error> {
        let http = reqwest::Client::builder()
            .timeout(config.timeout)
            .user_agent(format!("ironbridge-rust/{VERSION}"))
            .build()?;
        Ok(Self { config, http })
    }
...
pub const JAVA_SDK_TEMPLATE: &str = r#"// IronBridge Java SDK
// Auto-generated - Do not edit directly
// Version: {{version}}
//
// Requires Java 11+ (java.net.http). No external dependencies.

package ai.nervosys.ironbridge;
...
public class IronBridge {
    ...
    public static class Config {
        ...
        public Config() {
            String env = System.getenv("IRONBRIDGE_BASE_URL");
            this.baseUrl = (env == null || env.isEmpty()) ? "{{base_url}}" : env;
            this.apiKey = System.getenv("IRONBRIDGE_API_KEY");
        }
    }

    /** Thrown for any non-2xx response; carries the status so callers can branch. */
    public static class IronBridgeException extends RuntimeException {
        public final int statusCode;

        public IronBridgeException(String message, int statusCode) {
            super("ironbridge: " + message + " (status " + statusCode + ")");
            this.statusCode = statusCode;
        }
    }
    ...
    public static class Client {
        ...
        private String send(HttpRequest.Builder builder) {
            ...
            builder.header("User-Agent", "ironbridge-java/" + VERSION);
            ...
            try {
                ...
                if (resp.statusCode() >= 400) {
                    throw new IronBridgeException(resp.body(), resp.statusCode());
                }
                return resp.body();
            } catch (IronBridgeException e) {
                throw e;
            } catch (Exception e) {
                throw new RuntimeException("ironbridge: request failed", e);
            }
        }
...
pub const CSHARP_SDK_TEMPLATE: &str = r#"// IronBridge C# SDK
// Auto-generated - Do not edit directly
// Version: {{version}}
//
...
namespace Nervosys.IronBridge
{
    public class IronBridgeConfig
    {
        ...
        public IronBridgeConfig()
        {
            var env = Environment.GetEnvironmentVariable("IRONBRIDGE_BASE_URL");
            BaseUrl = string.IsNullOrEmpty(env) ? "{{base_url}}" : env;
            ApiKey = Environment.GetEnvironmentVariable("IRONBRIDGE_API_KEY");
        }
    }

    /// <summary>Thrown for any non-2xx response; carries the status code.</summary>
    public class IronBridgeException : Exception
    {
        public int StatusCode { get; }

        public IronBridgeException(string message, int statusCode)
            : base($"ironbridge: {message} (status {statusCode})")
        {
            StatusCode = statusCode;
        }
    }

    public class IronBridgeClient : IDisposable
    {
        public const string Version = "{{version}}";
        public const string ApiVersion = "{{api_version}}";

        private readonly IronBridgeConfig _config;
        private readonly HttpClient _http;

        public IronBridgeClient(IronBridgeConfig? config = null)
        {
            _config = config ?? new IronBridgeConfig();
            _http = new HttpClient { Timeout = _config.Timeout };
            _http.DefaultRequestHeaders.UserAgent.ParseAdd($"ironbridge-csharp/{Version}");
        ...
        private async Task<string> SendAsync(HttpRequestMessage request)
        {
            ...
            {
                throw new IronBridgeException(body, (int)response.StatusCode);
            }
            return body;
        }
...
pub const RUBY_SDK_TEMPLATE: &str = r#"# IronBridge Ruby SDK
# Auto-generated - Do not edit directly
# Version: {{version}}
#
...
module IronBridge
  ...
  class Error < StandardError
    ...
    def initialize(message, status_code)
      super("ironbridge: #{message} (status #{status_code})")
  ...
  class Config
    ...
    def initialize(base_url: nil, api_key: nil, timeout: 30)
      @base_url = base_url || ENV['IRONBRIDGE_BASE_URL'] || '{{base_url}}'
      @api_key = api_key || ENV['IRONBRIDGE_API_KEY']
      @timeout = timeout
    end
  end
  ...
  class Client
    ...
    def send_request(request, uri)
      ...
      request['User-Agent'] = "ironbridge-ruby/#{VERSION}"
...
/** Thrown for any non-2xx response; carries the status so callers can branch. */
class IronBridgeException extends \RuntimeException
{
    public int $statusCode;

    public function __construct(string $message, int $statusCode)
    {
        parent::__construct("ironbridge: {$message} (status {$statusCode})");
        $this->statusCode = $statusCode;
    }
}
...
class Config
{
    ...
    {
        $this->baseUrl = $baseUrl ?: (getenv('IRONBRIDGE_BASE_URL') ?: '{{base_url}}');
        $this->apiKey = $apiKey ?: (getenv('IRONBRIDGE_API_KEY') ?: null);
...
class Client
{
    ...
    private function send(string $method, string $path, ?array $body): array
    {
        $headers = [
            'Content-Type: application/json',
            'User-Agent: ironbridge-php/' . self::VERSION,
        ];
        ...
        if ($responseBody === false) {
            $error = curl_error($ch);
            curl_close($ch);
            throw new \RuntimeException('ironbridge: request failed: ' . $error);
        }
        $status = (int) curl_getinfo($ch, CURLINFO_HTTP_CODE);
        curl_close($ch);

        if ($status >= 400) {
            throw new IronBridgeException((string) $responseBody, $status);
        }
...
mod tests {
    ...
    fn every_sdk_talks_to_the_same_api_surface() {
        for language in SdkLanguage::ALL {
            ...
            assert!(
                sdk.contains("IRONBRIDGE_BASE_URL") && sdk.contains("IRONBRIDGE_API_KEY"),
