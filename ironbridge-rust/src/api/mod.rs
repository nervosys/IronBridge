...
//!
//! Provides a REST API for the web frontend and mobile app to interact with IRONBRIDGE.
...
//! * SAML signature verification is pure Rust, via `ironbridge-sso`, so the feature
...
pub use graphql::{configure_graphql_routes, create_schema, IronBridgeSchema};
...
pub struct ServerConfig {
    ...
    /// address. Set from `IRONBRIDGE_PUBLIC_BASE_URL`.
    ///
...
impl Default for ServerConfig {
    fn default() -> Self {
        Self {
            ...
            public_base_url: std::env::var("IRONBRIDGE_PUBLIC_BASE_URL")
                .ok()
...
pub async fn start_server(config: ServerConfig) -> Result<()> {
    ...
    println!("[*] IRONBRIDGE API Server starting...");
    ...
    let server = HttpServer::new(move || {
        ...
        // this server. So it is opt-in: `IRONBRIDGE_CORS_ALLOW_LOCALHOST=1` restores
        ...
        let allow_wildcard_localhost = std::env::var("IRONBRIDGE_CORS_ALLOW_LOCALHOST")
...
mod base_url_tests {
    ...
    fn an_explicit_public_url_overrides_the_bind_address() {
        assert_eq!(
            config("0.0.0.0", Some("https://ironbridge.example.com")).resolved_base_url(),
