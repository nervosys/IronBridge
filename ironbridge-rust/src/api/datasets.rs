...
//!
//! The two clients meant opposite things by the word. `ironbridge-app`'s Developer
//! screen modelled a dataset as the user's own export, with an **Upload**
//! button. `ironbridge-web`'s Developer page modelled it as a remote HuggingFace
    ...
    if body.entries.len() > MAX_ENTRIES {
        return bad_request(format!(
            "{} entries exceeds the {MAX_ENTRIES} this endpoint accepts in one \
             request. IronBridge has no streaming import yet.",
            body.entries.len()
        ));
    }
