...
pub async fn execute_tool(
    ...
    match body.tool.as_str() {
        ...
        "run_command" => {
            ...
            // network. It stays disabled until `IRONBRIDGE_ENABLE_RUN_COMMAND=1`, so
            // the capability is a deliberate choice, not a default.
            let enabled = std::env::var("IRONBRIDGE_ENABLE_RUN_COMMAND")
                .map(|v| v == "1" || v.eq_ignore_ascii_case("true"))
                .unwrap_or(false);
            if !enabled {
                return ApiResponse::<()>::error(
                    "run_command is disabled. Set IRONBRIDGE_ENABLE_RUN_COMMAND=1 in the server's \
...
mod run_command_gate_tests {
    ...
    /// `run_command` must refuse unless `IRONBRIDGE_ENABLE_RUN_COMMAND` is set.
    ///
    ...
    async fn run_command_is_refused_by_default() {
        ...
        assert!(
            std::env::var("IRONBRIDGE_ENABLE_RUN_COMMAND").is_err(),
            "IRONBRIDGE_ENABLE_RUN_COMMAND is set in the test environment; \
