...
//!
//! These back the twelve calls `ironbridge-web` was making against routes that did
...
pub async fn create_session(
    ...
    let provider = body.provider.clone().unwrap_or_else(|| "ironbridge".into());
...
/// `ironbridge analyze`: `OPENAI_API_KEY` and `OPENAI_BASE_URL`. Without a key this
...
pub async fn merge_sessions(
    ...
    let mut provider = String::from("ironbridge");
...
//
// That is a deliberate choice, not a shortcut. IronBridge holds a person's entire
