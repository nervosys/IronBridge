...
pub fn create_migration(
    ...
    for ws in &filtered {
        ...
        std::fs::write(ws_dir.join("workspace.json"), serde_json::to_string_pretty(&ws_json)?)?;
...
pub fn restore_migration(
    ...
    for ws in &manifest.workspaces {
        ...
        // Apply path mapping
        let new_path = ws.project_path.as_ref().map(|p| {
            path_map.get(p).cloned().unwrap_or_else(|| p.clone())
        });
