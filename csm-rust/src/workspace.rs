...
pub fn get_workspace_storage_path() -> Result<PathBuf> {
    let path = if cfg!(target_os = "windows") {
        dirs::config_dir()
            .map(|p| p.join("Code").join("User").join("workspaceStorage"))
    } else if cfg!(target_os = "macos") {
        dirs::home_dir()
            .map(|p| p.join("Library/Application Support/Code/User/workspaceStorage"))
    } else {
        // Linux
        dirs::home_dir()
            .map(|p| p.join(".config/Code/User/workspaceStorage"))
    };
...
pub fn get_workspace_by_hash(hash: &str) -> Result<Option<Workspace>> {
    ...
    Ok(workspaces.into_iter().find(|w| w.hash == hash || w.hash.starts_with(hash)))
}
...
pub fn find_workspace_by_path(project_path: &str) -> Result<Option<(String, PathBuf, Option<String>)>> {
    let storage_path = get_workspace_storage_path()?;
...
pub fn find_all_workspaces_for_project(project_name: &str) -> Result<Vec<(String, PathBuf, Option<String>, std::time::SystemTime)>> {
    let storage_path = get_workspace_storage_path()?;
