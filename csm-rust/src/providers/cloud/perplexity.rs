...
pub fn parse_perplexity_export(json_data: &str) -> Result<Vec<CloudConversation>> {
    ...
    Ok(threads.into_iter().map(|thread| {
        CloudConversation {
            ...
            updated_at: Some(parse_iso_timestamp(&thread.updated_at).unwrap_or_else(|_| Utc::now())),
            model: Some("perplexity".to_string()),
            messages: thread.messages.into_iter().map(|msg| {
                // Append sources to assistant messages
                let content = if !msg.sources.is_empty() && msg.role == "assistant" {
                    let sources_text = msg.sources.iter()
                        .enumerate()
