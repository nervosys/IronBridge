...
--
-- IronBridge Telescope extension
--
...
local session_previewer = previewers.new_buffer_previewer({
  ...
  define_preview = function(self, entry, status)
    local session = ironbridge.get_session(entry.value.id)
    if not session then
...
function M.sessions(opts)
  ...
  pickers
    .new(opts, {
      prompt_title = "IronBridge Sessions",
      ...
      attach_mappings = function(prompt_bufnr, map)
        ...
        actions.select_default:replace(function()
          ...
          if selection then
            ironbridge.view_session(selection.value.id)
          end
        end)
        ...
        map("i", "<C-h>", function()
          ...
          local new_sessions = ironbridge.get_sessions()
          picker:refresh(
            finders.new_table({
...
function M.search(opts)
  ...
  local sessions
  if query ~= "" then
    sessions = ironbridge.search(query)
  else
    sessions = ironbridge.get_sessions()
  end
...
return telescope.register_extension({
  ...
  exports = {
    sessions = M.sessions,
    search = M.search,
    ironbridge = M.sessions, -- Default action
  },
})
