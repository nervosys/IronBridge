...
async function main() {
    ...
    try {
        ...
        await page.screenshot({ path: 'ironbridge_web_home.png' });
        console.log("✓ ironbridge_web_home.png captured");
        ...
        for (const route of routes) {
            try {
                ...
                await page.screenshot({ path: `ironbridge_web_${route}.png` });
                console.log(`✓ ironbridge_web_${route}.png captured`);
            } catch (e) {
        ...
        await mobilePage.screenshot({ path: 'ironbridge_app_home.png' });
        console.log("✓ ironbridge_app_home.png captured");

        for (const route of routes) {
            try {
                await mobilePage.goto(`http://localhost:5174/${route}`, { waitUntil: 'networkidle', timeout: 5000 });
                await mobilePage.waitForTimeout(500);
                await mobilePage.screenshot({ path: `ironbridge_app_${route}.png` });
                console.log(`✓ ironbridge_app_${route}.png captured`);
            } catch (e) {
        ...
        await desktopPage.screenshot({ path: 'ironbridge_desktop_home.png' });
        console.log("✓ ironbridge_desktop_home.png captured");

        for (const route of routes) {
            try {
                await desktopPage.goto(`http://localhost:5174/${route}`, { waitUntil: 'networkidle', timeout: 5000 });
                await desktopPage.waitForTimeout(500);
                await desktopPage.screenshot({ path: `ironbridge_desktop_${route}.png` });
                console.log(`✓ ironbridge_desktop_${route}.png captured`);
            } catch (e) {
