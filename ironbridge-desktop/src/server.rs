...
//! Embedded API server.
//!
//! The desktop app loads `ironbridge-web`, which talks to the IronBridge REST API. On
...
//! run `ironbridge api serve` in a terminal. The window was real; the application
//! was not.
//!
...
/// `ironbridge api serve` alongside the app should not have the desktop app
...
/// application holding the port is not a IronBridge API, and adopting it would
...
pub async fn ensure_running(port: u16) -> ServerStatus {
    ...
    let config = ironbridge::api::ServerConfig {
        host: HOST.to_string(),
        port,
        ..Default::default()
    };
    ...
    std::thread::Builder::new()
        ...
        .spawn(move || {
            ...
            if let Err(e) = runtime.block_on(ironbridge::api::start_server(config)) {
...
mod tests {
    ...
    fn the_default_port_avoids_the_cli_default() {
        // 8787 is what `ironbridge api serve` uses; colliding by default would
    ...
    async fn embedded_server_actually_serves() {
        ...
        // Prove it is really serving IronBridge, not merely holding the port.
    ...
    #[tokio::test]
    async fn a_non_ironbridge_listener_is_not_adopted() {
        // Something holding the port that is not a IronBridge API must not be
