...
function! s:request(method, path, ...) abort
  let l:url = g:chasm_server_url . a:path
...
" Check server health
function! chasm#health() abort
  let l:data = s:request('GET', '/health')
  
  if has_key(l:data, 'error')
    echohl ErrorMsg
    echo 'Chasm server unreachable: ' . l:data.error
    echohl None
    return
  endif
  ...
  echohl Title
  echo 'Chasm server: ' . l:status . ' (v' . l:version . ')'
  echohl None
endfunction

" Harvest sessions
function! chasm#harvest() abort
...
" Search sessions
function! chasm#search(query) abort
  if empty(a:query)
