# IRONBRIDGE Mobile App

Cross-platform mobile app for Chat Session Manager (IRONBRIDGE). View and search your AI chat sessions from any device.
...
- npm or yarn
- Expo CLI (`npx expo`)
- IRONBRIDGE API server running on `localhost:8787`
...
### 2. Start the IRONBRIDGE API server

```bash
# From ironbridge-rust directory
cargo run --bin ironbridge -- api serve
```
...
The app connects to the IRONBRIDGE API server. The base URL is configured in `src/api/client.ts`:
