...
// =============================================================================
// IRONBRIDGE Introspection (MCP Tools)
// =============================================================================
...
// Fetch available IRONBRIDGE tools
export async function getIronBridgeTools(): Promise<McpTool[]> {
    const { apiClient } = await import('./client');
    try {
        const response = await apiClient.get('/mcp/tools');
        return response.data?.mcp_tools || [];
    } catch (error) {
        console.error('Failed to fetch IRONBRIDGE tools:', error);
        return [];
    }
}

// Get system prompt with IRONBRIDGE context
export async function getIronBridgeSystemPrompt(): Promise<string> {
    const { apiClient } = await import('./client');
    try {
        const response = await apiClient.get('/mcp/system-prompt');
        return response.data?.system_prompt || '';
    } catch (error) {
        console.error('Failed to fetch IRONBRIDGE system prompt:', error);
        return '';
    }
}

// Execute a IRONBRIDGE tool call
export async function callIronBridgeTool(name: string, args: Record<string, unknown>): Promise<ToolCallResult> {
    const { apiClient } = await import('./client');
    try {
        ...
        console.error('Failed to call IRONBRIDGE tool:', error);
        return {
            tool: name,
            result: {
...
// Send chat completion with IRONBRIDGE tools support
export async function sendChatCompletionWithTools(
    request: ChatCompletionRequest & { enableIronBridgeTools?: boolean }
): Promise<ChatCompletionResponse & { toolCalls?: Array<{ name: string; arguments: string }> }> {
    const { provider, messages, temperature = 0.7, maxTokens = 4096, enableIronBridgeTools } = request;

    // Get IRONBRIDGE tools if enabled
    let tools: Array<Record<string, unknown>> = [];
    let systemPrompt: string | undefined;

    if (enableIronBridgeTools) {
        const csmTools = await getIronBridgeTools();
        tools = csmTools
            .filter(t => t.name.startsWith('ironbridge_db_')) // Only database tools for chat
            .map(t => ({
                type: 'function',
                function: {
                    name: t.name,
        ...
        systemPrompt = await getIronBridgeSystemPrompt();
    }

    // Build messages with system prompt if IRONBRIDGE tools enabled
