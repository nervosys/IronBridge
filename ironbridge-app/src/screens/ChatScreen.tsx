...
export function ChatScreen({ navigation }: Props) {
    ...
    const [csmToolsEnabled, setIronBridgeToolsEnabled] = useState(true);
    ...
    const handleSend = async () => {
        ...
        try {
            ...
            // Use tools-enabled completion if IRONBRIDGE tools are on and provider supports it
                ...
                if (response.toolCalls && response.toolCalls.length > 0) {
                    ...
                    for (const toolCall of response.toolCalls) {
                        try {
                            ...
                            const result = await callIronBridgeTool(toolCall.name, args);
    ...
    return (
        ...
        >
            {/* Provider subtitle under header with IRONBRIDGE tools toggle */}
            <View style={[styles.providerBar, { backgroundColor: colors.card, borderBottomColor: colors.divider }]}>
                ...
                <TouchableOpacity
                    ...
                    onPress={() => setIronBridgeToolsEnabled(!csmToolsEnabled)}
                >
                    <Ionicons
                        name="library"
                        size={14}
