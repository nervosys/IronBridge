...
class BiometricAuthService {
    ...
    private getDefaultPromptMessage(): string {
        if (!this.status?.supportedTypes.length) {
            return 'Authenticate to access Chasm';
        }
        ...
        switch (primaryType) {
            case 'facial':
                return Platform.OS === 'ios' ? 'Use Face ID to unlock Chasm' : 'Use face recognition to unlock Chasm';
            case 'fingerprint':
                return Platform.OS === 'ios' ? 'Use Touch ID to unlock Chasm' : 'Use fingerprint to unlock Chasm';
            case 'iris':
                return 'Use iris scan to unlock Chasm';
            default:
                return 'Authenticate to access Chasm';
        }
    }
